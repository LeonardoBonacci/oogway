...
public class ElasticAdapter<T extends BaseObject> {
	...
	public ElasticAdapter<T> thePipeline(String pipeline) {
	...
	public Mono<T> findById(String id) {
		return Mono.<GetResponse>create(
				sink -> client.getAsync(new GetRequest(index, type, id), RequestOptions.DEFAULT, listenerToSink(sink)))
				.filter(GetResponse::isExists).map(resp -> {
	...
	public Mono<T> searchOne(SearchSourceBuilder search) {
		return Mono
				.<SearchResponse>create(sink -> client.searchAsync(new SearchRequest(index).types(type).source(search),
						RequestOptions.DEFAULT, listenerToSink(sink)))
				.map(SearchResponse::getHits).filter(hits -> hits.totalHits > 0)
				.map(hits -> hits.getAt(new Random().nextInt(hits.getHits().length))).map(SearchHit::getSourceAsMap)
	...
	public Flux<T> search(SearchSourceBuilder search) {
		return Flux
				.<SearchResponse>create(sink -> client.searchAsync(new SearchRequest(index).types(type).source(search),
						RequestOptions.DEFAULT, listenerToSink(sink)))
				.map(SearchResponse::getHits).flatMap(hs -> Flux.fromArray(hs.getHits())).map(resp -> {
	...
	private void doIndex(T doc, ActionListener<IndexResponse> listener) throws JsonProcessingException {
		...
		if (!isEmpty(pipeline)) request.setPipeline(pipeline);
	...
	private <U> ActionListener<U> listenerToSink(FluxSink<U> sink) {
		return new FluxSinkListener<U>(sink);
	}
}
