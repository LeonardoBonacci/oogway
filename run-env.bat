...
docker run -d --name logstash --link es:elasticsearch --rm --link rabbit:rabbitmq -it -v C:/Users/Leo/git/oogway/logstash/pipeline/:/usr/share/logstash/pipeline/ -v C:/Users/Leo/git/oogway/logstash/config/pipelines.yml:/usr/share/logstash/config/pipelines.yml -p 4560:4560 leonardobonacci/pruning-logstash:1.0
